...
def show_radiologist():
    ...
    registration_number = (
        doctor.get("registration_number")
        or ""
    )
    ...
    for item in requests:
        ...
        request_options.append(
            f"{item.get('id')} | Scan: {item.get('scan_id')}"
        )
    ...
    remarks = st.text_area(
        "Radiologist Remarks",
        height=110,
        placeholder="Additional professional remarks...",
        ...
        try:
            ...
            report_buffer, report_id = (
                generate_pdf_report(
                    ...
                    remarks=remarks.strip(),
                    reviewed_at=reviewed_at.strftime(
                        "%d %B %Y, %H:%M"
                    ),
                    xray_image=(
                        scan_image_bytes
                        if scan.get("examination")
                        == "Chest X-ray"
                        else None
                    ),
                    ultrasound_image=(
            ...
            if not report_response.data:
                ...
                st.error(
                    "The PDF was generated, but "
                    "the medical report could not be saved."
                )
